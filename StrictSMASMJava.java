/*
 * Java Programming Assignment #2  Spring 2024 – Parser (Strictly SMASM Version)
 * Description: This program reads a file of tokens, parses the tokens based on grammar rules, and notifies the user if the program is legal or not. This works exactly as shown on page 4 of assignment instructions.
...
public class StrictSMASMJava {
    ...
    public static boolean COMMANDS(Scanner inStream, PrintWriter outStream) {
         ...
         *              With this version, it strictly adheres to the intended cases we've viewed in writing; it builds on the grammar rules.
         *              This program will match the commands as well with the other provided non-terminals and terminals to ensure the program is legal.
         *              The previous version would have allowed for any command to be entered for the COMMANDS grammar rule, but this version is more strict and will only allow certain commands as shown in example
         *              programs and page 5 of the assignment instructions.
         *              The method prints to the terminal and logs to the output file to track the flow of execution.
         * 
         *              COMMANDS -> COMMAND ‘[‘ (REGISTER | VALUE )  [ ‘,’ REGISTER] ‘]’
         * 
        ...
        */
        ...
        if (COMMAND(inStream, outStream, command)) { // if the COMMAND method returns true given the command token
            ...
            if (REGISTER(outStream, registerValueLine) == 1) { // if the REGISTER method returns 1 given the registerValueLine
                try {
                    String nextLine = getNextLine(inStream); // get the next line in the input file
                    if(nextLine.equals("]") && (command.equals("sq") || command.equals("rt"))){ // check if end early before the optional portion
                ...
                if (REGISTER(outStream, toLower(getNextLine(inStream))) == 1) { // if the REGISTER method returns 1 given the registerValueLine
                    try {
                        String nextLine = getNextLine(inStream); // get the next line in the input file
                        if(nextLine.equals("]") && (command.equals("sub") || command.equals("st") || command.equals("add"))){ // check if command is valid for the early end bracket
                            System.out.println("Leaving function: COMMANDS Success");
                            outStream.println("Leaving function: COMMANDS Success");
                            return true;
                        }
                        else { // if the next line is not ']', and/or the command is not valid for the early end bracket
                            System.out.println("Leaving function: COMMANDS Unsuccessful");
                            outStream.println("Leaving function: COMMANDS Unsuccessful");
                            return false;
                        }
                ...
                if (!command.equals("ld")){ // if it is not an ld command, return false as this is only valid for ld
                    System.out.println("Leaving function: COMMANDS Unsuccessful");
                    outStream.println("Leaving function: COMMANDS Unsuccessful");
                    return false;
                }
                ...
                try {
                    ...
                    if (nextLine.equals(",")) { // if the next line is a comma
                        isComma = true;
                    } 
                    else {
                        isComma = false;
                ...
                if (isComma) {
                    ...
                    registerValueLine = toLower(getNextLine(inStream)); // get the next line in the input file after the comma and convert it to lowercase

                    if (REGISTER(outStream, registerValueLine) == 1) {
                        try {
                            String nextLine = getNextLine(inStream); // get the next line in the input file
                            if (nextLine.equals("]")) { // if the next line is ']'
                                System.out.println("Leaving function: COMMANDS Success");
                                outStream.println("Leaving function: COMMANDS Success");
                                return true;
                            } 
